(function () {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        for (var j = 0; j < 4; j++) {
          ...
          if (!isMobile) {
            ...
            div.style.top = 100 * i + 'px';
            div.style.left = 100 * j + 'px';
          }
          tr.appendChild(div);
          // tr.appendChild(td);
        }
    ...
    function updateTable(direct) {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        // 如果是empty单元格，就跳过
        // 如果不是empty，如果有下一个元素且相同，当前元素移动长度加1，下一个元素添加求和动画
        if (order) {
          for (var j = 3; j >= 0; j--) {
            if (tc[j]) {
              if (j - 1 >= 0) {
      ...
      // for (var i = 0; i < 16; i++) {
      //   if (move[i] || addMove[i]) {
      //     tds[i].classList.add('move' + direct + move[i] + '-addMove' + addMove[i]);
      //   }
      //   if (adds[i]) {
      ...
      // tds[random2Pos].classList.add('add2');
        ...
        tds.forEach((elem, index) => {
          ...
          if (elem.innerHTML) {
            ...
            if (index === random2Pos) {
              ...
              else elem.classList.add('add' + elem.innerHTML);
            }
          }
        });
        document.body.appendChild(frag);
